...
### 2026-08-27（日）文档中英分置 + 前后台样式修正（目录树对齐/挂载切换/移动端宽度/表格排版）
- **文档中英分置**：vault/BrainPress 五篇文档（Introduction/GettingStarted/Configuration/Deployment/API-Reference）改为英文 `*.md` + 中文 `*.zh.md` 同目录双份，内容对齐当前实现；ChangeLog 保持中文开发日志不动
- **后台侧栏菜单对齐首页目录树**：`#left-drawer-md`/`#drawer-md` 的 6 视图顶层项字号字重由 15px/600 提到 17px/700，与首页一级目录（`li.has-children`）一致（admin.css）
- **挂载页分段切换不截断**：`.toggle-opt` 改 `white-space:nowrap`（移除 overflow/ellipsis 裁剪），手机端 `≤560px` 缩字号+减内边距（admin.css）
- **移动端内容宽度响应式**：中宽屏（769–1100px）gutter 48→24、侧栏 280→240；手机端（≤768px）gutter 14、内边距收到 14——解决中宽屏走桌面布局导致内容被压成居中窄条（viewport meta 本就存在，非缺失）（admin.css）
- **核心功能表格排版**：单元格 `line-height` 由继承的 25px 收到 1.5（描述列不再比功能列疏）；新增 `.md th/.md td * { font-size:inherit }` 强制 code/strong/mark 继承单元格字号，杜绝列间视觉差（site.css）
- **iOS 文字放大根治**：`html,body` 补 `-webkit-text-size-adjust:100%`——手机端表格 nowrap 长行溢出时 Safari 会自动放大文字（"又粗又大"），现已禁止；手机端表格保持 `white-space:nowrap` 横向滚动（用户要求不强制换行）（site.css）
- **目录树两色/对齐（已回退）**：曾尝试把 `.dir-arrow` 改绝对定位+同色（`--vp-c-text-1`）、预留箭头槽让目录/文件标签同列对齐；用户要求回退，site.css 与 admin.css 均已还原，仅保留上述移动端宽度修复
