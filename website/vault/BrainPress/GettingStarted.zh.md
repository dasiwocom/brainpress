...
BrainPress 还内置了几个开箱即用、无需配置的辅助功能：

- **Mermaid 图表**：在笔记里用 ```` ```mermaid ```` 代码块写图（流程图、时序图、思维导图、甘特图），打开笔记时会自动渲染成图形，而不是灰色代码框。示例：
  ````
  ```mermaid
  graph LR
      A[开头] --> B[处理] --> C[结束]
  ```
  ````
- **链接悬停预览（Popover）**：把鼠标悬停在内部链接（`[[双链]]` 或普通笔记链接）上，会浮现一个小卡片预览目标笔记的开头，不必点进去。
- **面包屑导航**：文章顶部显示当前位置路径，点击任意上层目录可展开侧边栏树定位。
- **最近笔记**：首页底部列出最新修改的若干篇笔记，快速回到最近工作。
- **标签页**：点击任意 `#标签` 会进入一个独立整页，列出所有带该标签的笔记。

### RSS 订阅与 Sitemap

对外发布时还有两个给"读者 / 搜索引擎"用的地址（详见部署文档的"对外订阅与收录"小节）：

- **`/rss.xml`**：RSS 订阅源——别人可把此地址填入阅读器（如 Feedly），自动收到你的新文章，无需手动刷新。
- **`/sitemap.xml`**：站点地图——提交给 Google 搜索控制台 / 百度站长平台，让爬虫按清单收录你的公开文章。
