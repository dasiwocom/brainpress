...
BrainPress exposes a small, plain-HTTP API. Everything is JSON (UTF-8), and the read endpoints are **public** — no authentication required, so any script or tool can consume the knowledge base. Admin endpoints require a session cookie obtained from `/api/login` (or `/api/setup` on first run).
...
Reads one note's **raw Markdown content** (server-side render pipeline not applied). Works for `.md` notes. For `.excalidraw.md` and `.canvas` files it returns the raw source as well.
...
### `POST /api/setup` — first-run password (no auth)

Sets the admin password when none is configured yet.
...
Password must be at least 4 characters. If a password is already set, returns `400` with `ok:false`.
...
Returns the full configuration: mounts, render toggles, custom paths, exclude/pinned/expanded lists, site title, home article, theme defaults, font preset, graph settings.
...
Scans every note's title and content for the query (case-insensitive), returns path + name + a snippet around the first hit. Hidden paths are excluded. Covers `.md` and `.canvas` content.
...
Returns the nodes and edges used by the `/graph` view: every note is a node, wiki links (`[[...]]`) are edges. Useful for external visualization too.
...
- The write API targets the main `vault/` only (mounted paths are intentionally excluded).
...
Answers a question using the knowledge base (retrieval over `vault/` + an OpenAI-compatible LLM). Requires the AI master switch on and an API key / base URL configured (admin panel → AI). Read-only — never modifies content.
...
| Empty question | `400` |
| LLM error | `502` with error message |
...
`.pdf` paths work the same way: requesting a vault PDF by URL returns the application shell with the built-in reader pre-loaded for that document (no download prompt) — the same shared reader UI used by `![[book.pdf]]` embeds inside notes.

`.excalidraw.md` and `.canvas` paths also return a fully rendered page: the Excalidraw drawing (official engine SVG) or the Canvas whiteboard (SVG scene) is pre-rendered server-side, so these views work without waiting for client-side rendering.
